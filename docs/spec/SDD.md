...
type WaterKind = '公园浅湖' | '大水面' | '收费塘' | '路亚塘' | '内河' | '河口';
type WaterColor = '未知' | '瘦清' | '黄绿' | '肥浊' | '泥浆' | '恶水';
type PondCare = '未知' | '刚换水' | '换水回稳' | '刚调水消毒' | '老水';
...
type FishingIndex = {
  ...
  reasons: string[];
  waterBite: string; // 开口较好 / 偏挑 / 差 / 尚可 / 不宜，来自 WATER_BITE_SCORE
};
...
纯函数 `buildAdvice(weather, at = Date)`。口径对齐国内淡水教材与上海二次检索（钓鱼之家：气压走低口差、升压回口，近海平面 1006–1028 好钓；酷钓鱼：海拔 0 米参考 ≥1005 hPa；渔钓者：上浮为找氧；水产增养殖学：鲫鲤底层、草中下、鲌中上），不用欧美降压抢食。优先级从高到低：

1. **盛夏正午高温**（月∈{6,7,8,9} 且气温≥30 且小时∈[10,16]）→ 底层；清淡底饵；建议荫凉/夜钓。若同时气压急降或低压（≤1005），改**中下层荫凉/进水口**，不死守亮水底（表层晒烫、静水底也可能闷）。台钓主攻草鱼/鳊鱼时改为中上层（夏天浮钓草鱼的国内经验）。
2. **气压急降**（3h ΔP ≤ -1.5 hPa）→ 中上层找氧；轻质拉饵、小钩细线；台钓改浮或半水、路亚放慢搜中上。文案写口差，不写抢食。
3. **高气压稳定**（气压 ≥ 1022 且 |ΔP| < 1）→ 底层；蚯蚓红虫或搓饵守底。高压稳压按国内经验当作相对好钓底，不写口轻。
4. **低气压**（气压 ≤ **1005** hPa，上海近海平面）→ 中上层找氧；轻饵、进出水口与下风；浮头则建议收竿。口差，不是更活跃。1006–1008 不再当低压口差。
5. **默认** → 中下层；香腥各一；台钓找底后略离底。升压时窗口写「气压回升」。

附加修正：

- 降水中：偏腥、靠近进水口，层略升一档（底层→中下层，中下层→中上层）。小雨按国内「斜风细雨」加分，雷暴仍不宜。
- 风力按中国气象几级，不按欧美 mph。**3–4 级**国内淡水常是好风，指数不因此减分。**4 级起**战术改加重饵、抗风钓组、路亚侧风岸、台钓下风口。**5 级及以上**抛投与找口变难。盛夏 **0–1 级**水面易闷。盛夏相对湿度 ≥85% 且风力 ≤2 级记闷湿，不写溶氧数字。
- 小时 ∈ [5,7]∪[17,19]：可并列推荐浅层对象鱼（白条/翘嘴）。`night` 为小时 < 5 或 ≥ 20，与晨昏窗错开，避免凌晨五点被当成夜钓勺型。
...
`buildAdvice(weather, at, { targetFish, style, waterKind, waterColor, pondCare })`。`src/lib/plan.ts` 给出味型、饵形、标点；`src/lib/water.ts` 给出水域/水色/塘保养：

- 味型：气温＜12 大腥；12–18 腥香；18–26 香腥；≥26 清香；盛夏正午或≥30 本味清淡。低压清淡改「清淡带果酸」（国内轻口）。不因气压急降改成抢食腥饵。水色瘦清改偏腥香；肥浊/泥浆改本味或粮食清香；刚换水/刚调水改清淡轻口。来源：公开台钓饵料文（冬春主腥、夏主清淡）、渔钓者观水色用饵。
- 饵形：鲫/鳊/白条偏拉饵；鲤/草/青或高气压/大风偏搓饵；草/鳊气温≥26 改颗粒/玉米；黄颡/鲶用虫饵；路亚为拟饵。刚换水/调水改软粘轻口；老水或肥浊改轻搓离底。
- 台钓标点：鲫→草边凹岸/草洞；鲤→凸岸缓坡亮水；草→草边；青→深潭桥墩；翘嘴/白条→深浅交界；黑鱼→草洞。雨天加进水口缓流；正午改荫凉桥洞；晨昏近岸浅滩。**收费塘**正午改荫凉边/增氧机；刚换水走进水口轻口；老水略离底避酱层。河口鲈/鲻走潮沟闸口。
- 路亚拟饵（`planLurePick`）：按对象鱼 + 气温/时段/降水 + 水域/水色。翘嘴上海公园浅湖夏 **5–7g** 斜切亮片、**大水面 7–12g**、冬春 12–20g 远投、晨昏波扒、夜钓勺型 7–10g；黑鱼草区 10–14g 雷蛙走停、**中鱼后等两三秒再抽**；鳜铅头钩卷尾 7–10g（白天跳底诱攻，不默认中上层巡游）/夜 VIB；鲈按上海路亚塘大口黑鲈给结构软虫或浅层米诺，近岸轻荡不必远投；浊水/泥浆改红头金或加震、贴障碍。白条瓜子亮片 1.5–3g；鲶胡须佬贴底。水色未知时仍：清水银白、浊水（雨）红头金、夜钓橙红。来源：渔夫者/钓鱼007、小浪纹/碧溪上上海浅湖、渔钓者、酷钓鱼、酷米网。运行时不联网。
- 水色指数（开口程度）：查表 `WATER_BITE_SCORE`（何志辉藻相 + 浊度实验）。黄绿/茶褐 +6（较好）；肥浊 -6（偏挑，盛夏无风再 -4）；恶水 -28 且分数封顶 34（不宜）；瘦清默认 0（一般），视觉猎手（鲈/翘嘴/白条/红鳍鲌/鳡/黑鱼，或未点鱼的路亚）+4；泥浆默认 -10，视觉 -12，底栖鲫鲤鲶黄颡塘鲺青 -6，鳜 -4。指数输出 `waterBite` 文案。塘保养仅收费塘/路亚塘：刚换水 -12，刚调水 -10，回稳 +4，老水 -4。未知不加不减。不把养殖光色实验写进水色芯片。

运行时不联网。

## 4b. 钓鱼指数（FR-8）

纯函数 `buildFishingIndex(weather, at = Date, water)`，起点 62 分，再按与建议引擎相同的气象信号加减，水色开口再按 `targetFish` / `style` 分叉，最后夹紧到 0–100。
...
档位：≥80 很高，≥65 较高，≥50 一般，≥35 偏低，否则不宜。理由 2–5 条，用气压/气温/风/水色开口解释。
...
窗口内 `phase=in`，倒计时到该窗结束；窗外 `phase=wait`，倒计时到下一窗开始（20:00 之后算次日晨间窗口）。`climateFlags.night` 为小时 < 5 或 ≥ 20，与窗口错开：晨昏给波扒，20:00 后才换夜钓勺型。文案：`晨间窗口剩余` / `黄昏窗口剩余` / `距晨间窗口` / `距黄昏窗口`。剩余时间格式 `HH:MM:SS`。首页天气条每秒刷新；不调模型、不联网。`fishGuide(name)` 返回习性介绍与技巧条目，词表外回落通用说明。
...
- `planLurePick`：上海浅湖翘嘴夏 5–7g、大水面 7–12g、冬春 12–20g、夜钓 7–10g。
- `waterColorDelta` / `pondCareDelta` / `venueToWaterKind`：黄绿加分、泥浆减分、刚换水只在收费塘生效、垂钓园映射收费塘。
- `buildFishingIndex`：气压急降压分、升压/高压稳加分、低压压分（≤1005）、盛夏正午压分、雷暴不宜、结果夹紧 0–100。
